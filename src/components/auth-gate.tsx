...
// flip to true once the Google OAuth provider is enabled in Supabase (Auth → Providers → Google)
const GOOGLE_ENABLED = false;
// magic link needs custom SMTP (default email is rate-limited); enable when SMTP is wired
  ...
  return (
    <main className="relative flex flex-1 items-center justify-center overflow-hidden bg-mesh px-5 py-16">
      ...
      <div className="relative w-full max-w-sm rounded-3xl border border-border bg-surface/70 p-8 backdrop-blur-xl">
        ...
        {/* Google — shown only once the provider is enabled in Supabase */}
        {GOOGLE_ENABLED && (
          <>
            <button
              onClick={google}
